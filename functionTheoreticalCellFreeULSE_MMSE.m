function [SE_CC,A] = functionTheoreticalCellFreeULSE_MMSE( HMean_Withoutphase,R_AP,Lk,Omega,Phi,M,K,N,tau_p,tau_c,p,Pset)       

%---This function is used to computes the theoretical uplink SE for
%phase-aware MMSE estimator.
...
for k = 1:K
    ...
    for m = 1:M
        ...
        Zk(m,m,k) = trace(p(k)*tau_p*Omega(:,:,m,k)+Lk(:,:,m,k));
...
%            if l ==k
%                
%                X_p2_Lk(m,m,k,l) = (HMean_Withoutphase((m-1)*N+1:m*N,l))'*HMean_Withoutphase((m-1)*N+1:m*N,l);
%                X_p2_Omeg(m,m,k,l) = trace(Omega(:,:,m,l));
%            end
...
for k = 1:K
    ...
    A_MMSE = sum(A_MMSE_part1(:,:,k,:),4) + Zk(:,:,k);
    A(:,:,k) = diag(A_MMSE\diag(Zk(:,:,k)));
